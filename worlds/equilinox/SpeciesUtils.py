...
def get_species_from_category(cat: str) -> list[Species | None]:
    list_str = []
    match cat:
        case "Animals": #If we just need a generic "animal", it suffices to just check the base species
            return [s for s in all_species if (not s.is_plant and s.is_base_species())]
        case "Plants": #If we just need a generic "animal", it suffices to just check the base species
            return [s for s in all_species if (s.is_plant and s.is_base_species())]
        case "Trees":
    ...
    return [get_species_from_name(s) for s in list_str]


def get_required_species_from_string(string: str) -> list[list[Species | None]]:
    required : list[list[Species | None]] = []
    species = string.split(";")
    for species in species:
        species = species.split(" (")[0]
        if get_species_from_name(species) is not None:
            required.append([get_species_from_name(species)])
        elif species == "Hive": #TODO: More
            required.append([get_species_from_name("Bee")])
        elif species == "Beaver Lodge": #TODO: More
            required.append([get_species_from_name("Beaver")])
        else:
